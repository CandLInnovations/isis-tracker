...
export default function TopicalPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    const { error } = await supabase.from('topical_logs').insert({
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="card">
            <p className="section-title">Log Application</p>
            <form onSubmit={submit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-1.5 mt-1">
                  {TOPICAL_PRODUCTS.map(p => (
                    <label key={p} className="flex items-center gap-2 cursor-pointer group">
                      <div
                        className={`w-4 h-4 flex-shrink-0 rounded border-2 flex items-center justify-center transition-all ${
                          selectedProducts.has(p)
                            ...
                            : 'border-bark-300 group-hover:border-moss-400'
                        }`}
                      ...
                      <span
                        className="text-sm font-serif text-bark-800 cursor-pointer"
                        onClick={() => toggleProduct(p)}
                      >
                        {p}
                      </span>
              ...
              <div>
                ...
                <textarea
                  className="textarea"
                  rows={3}
                  placeholder="Any observations, Isis's reaction, areas treated…"
                  value={notes}
        ...
        <div className="lg:col-span-3">
          ...
          {loading ? (
            ...
            <div className="card text-center">
              <p className="text-bark-400 italic text-sm py-6">No applications logged yet.</p>
            </div>
          ) : (
                    ...
                    {grouped[date].map(log => (
                      <div key={log.id} className="card">
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                          {log.products.map(p => (
                            <span key={p} className="text-xs bg-bark-100 text-bark-600 px-2 py-0.5 rounded-full font-serif">
                              {p}
                            </span>
                          ))}
                        </div>
                        {log.notes && (
                          <p className="text-xs text-bark-500 italic border-t border-bark-100 pt-2 mt-2">
                            {log.notes}
                          </p>
                        )}
                      </div>
