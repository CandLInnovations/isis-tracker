...
export default function FenbenPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (settings.data?.[0]) setCycleStart(settings.data[0].cycle_start_date)
  ...
  async function logDose(date: string, given: boolean) {
    await supabase.from('fenben_doses').upsert({ dose_date: date, given, notes: logNote || null }, { onConflict: 'dose_date' })
    setDoses(prev => {
  ...
  function getDayClass(ds: string): string {
    ...
    if (isPast)        return `bg-amber-100 text-amber-700 ring-1 ring-amber-400${ring}`
  ...
  const selectedDay   = cycleStart ? getCycleDayNumber(cycleStart, logDate) : null
        ...
        <div className="space-y-4">
          ...
          {cycleStart && (
            <div className="card">
              ...
              {!todayIsOn && todayCycleDay && (
                <p className="text-xs text-bark-500 italic mt-2">Next ON period begins in {7 - todayCycleDay} day(s).</p>
              )}
            </div>
          )}
          ...
          {cycleStart && (
            <div className="card">
              ...
              <input type="date" className="input mb-3" value={logDate} onChange={e => { setLogDate(e.target.value); setEditingDate(null) }} />
              ...
              {selectedDay !== null && selectedDay >= 1 && (
                <div className={`flex items-center gap-2 px-2 py-1.5 rounded-md mb-3 text-xs font-serif ${selectedIsOn ? 'bg-moss-100 text-moss-700' : 'bg-bark-100 text-bark-500'}`}>
                  ...
                  <span>{selectedIsOn ? `ON Day ${selectedDay}` : 'OFF / Rest Day'}{logDate === today ? ' (today)' : ''}</span>
                  {selectedEntry && (
              ...
              {!selectedIsOn && <p className="text-xs text-bark-400 italic mb-3">Rest day — you can still log if needed.</p>}
