...
export default function MedicationsPage() {
  ...
  function startEdit(log: GabaLog) {
    setEditingId(log.id)
    setGivenAt(new Date(new Date(log.given_at).getTime() - new Date(log.given_at).getTimezoneOffset() * 60000).toISOString().slice(0, 16))
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (editingId) {
      await supabase.from('gabapentin_logs').update(payload).eq('id', editingId)
    } else {
  ...
  async function deleteEntry(id: string) {
    if (!confirm('Delete this gabapentin log entry?')) return
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="card">
            <p className="section-title">{editingId ? 'Edit Entry' : 'Log Dose'}</p>
            {editingId && (
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                ...
                <textarea className="textarea" rows={2} placeholder="Additional context…" value={notes} onChange={e => setNotes(e.target.value)} />
              </div>
              <button type="submit" disabled={submitting} className="btn-primary w-full">
                {submitting ? 'Saving…' : editingId ? 'Update Entry' : 'Log Dose'}
              </button>
            </form>
          </div>
              ...
              {sortedDates.map(date => (
                <div key={date}>
                  <p className="text-xs text-bark-500 uppercase tracking-wide font-serif mb-2">
                    ...
                    <span className="ml-2 normal-case font-normal">({grouped[date].reduce((s, l) => s + l.pills, 0)} pills total)</span>
                  </p>
